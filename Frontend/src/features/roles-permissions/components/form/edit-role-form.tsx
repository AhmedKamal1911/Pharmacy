...
const formSchema = z.object({
  roleName: z.enum(["مدير النظام", "مدير", "موظف مبيعات"], {
    message: "يجب اختيار اسم دور صالح من القائمة.",
  }),
  ...
  description: z.enum(
    [
      "صلاحيات كاملة على النظام (دور فريد)",
      "صلاحيات إدارية واسعة على النظام",
      "صلاحيات محدودة للمبيعات وخدمة العملاء",
    ],
    {
      message: "يجب اختيار وصف صالح من القائمة.",
    },
  ),
  permissions: z
...
export type EditRoleFormData = z.infer<typeof formSchema>;
          ...
          render={({ field, fieldState }) => (
            <Field data-invalid={fieldState.invalid} className="space-y-1.5">
              ...
              >
                <SelectTrigger
                  id="role-name"
                  className={`focus-visible:ring-indigo-500 ${fieldState.invalid ? "border-destructive" : "border-input"} ${role.code === "SUPER_ADMIN" ? "bg-muted cursor-not-allowed" : ""}`}
                >
                  <SelectValue placeholder="اختر اسم الدور" />
                </SelectTrigger>
                <SelectContent>
